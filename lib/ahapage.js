  ...
  window.MathJax = {
    ...
    AuthorInit: function () {
      ...
      MathJax.Hub.Register.StartupHook("SVG Jax Config",function () {
        ...
        var backPostTranslate = SVG.postTranslate
        

        // SVG.postTranslate.push(aaa)
        MathJax.Callback.After(function () {
                    SVG.postTranslate = SVG.postProcess= function (state, partial) {
                        var scripts = state.jax[this.id];
                        for (var i = state.SVGlast, m = state.SVGeqn; i < m; i++) {
                          var script = scripts[i];
                          if(script && script.type == 'math/tex'){
                              var id = script.id+'-Frame';
                              var tmp = MathJax.HTML.Element('span', {className: 'MathJax_SVG_Source'}, script.innerHTML);
                              document.getElementById(id).appendChild(tmp);
                          }
                      }
                        
                        return backPostTranslate.apply(SVG, [].splice.call(arguments,0))
                    }
                })
        //
